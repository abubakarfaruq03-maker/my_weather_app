...
import { MdSearch } from 'react-icons/md' // Import the Material Design Search icon
...
function App() {
  ...
  // Single search function used by both button and Enter key
      ...
      <div className='flex flex-col justify-center items-center gap-10'>
        ...
        <div className='flex flex-col justify-center items-center gap-2 '>
          <form 
            ...
            className='flex justify-center items-center lg:mt-10 mt-20 group'
          >
            <div className="relative flex items-center">
              <input 
                className='p-3 pr-12 text-[16px] text-white border border-[rgba(255,255,255,0.8)] rounded-[16px] bg-transparent focus:outline-none focus:border-white transition-all w-64 sm:w-80'
                type="text" 
              ...
              <button 
                type="submit"
                className='absolute right-2 p-2 text-white/70 hover:text-white transition-colors cursor-pointer'
              >
                <MdSearch size={28} />
              </button>
            </div>
          ...
          <div className='mt-10 mr-8 sn:mr-10 flex flex-col justify-center items-center gap-4'>
            ...
            {Error && <p className='text-red-400 text-[1.5rem] font-semibold text-center mt-2 ml-10'>{Error}</p>}
            {weatherData?.main ? (
              <h1 className='text-[4rem] text-white text-center'>{weatherData.main.temp.toFixed()}°C</h1>
            ) : null}
          </div>
        </div>
        ...
        <div className='flex flex-row sm:flex-col justify-center gap-12 mr-2 items-center'>
          <div>
            ...
            {weatherData.weather ? (
              <p className='text-white font-bold text-2xl'>{weatherData.weather[0].main}</p>
            ) : null}
          </div>
